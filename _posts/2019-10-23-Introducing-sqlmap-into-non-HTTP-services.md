...
Recently, I was introduced to an interesting problem: How to automate the SQL injection exploitation of non-HTTP service? Non-HTTP service, in this case, means that the communication between client and server isn't HTTP based and might be text or binary. As `sqlmap` is designed to scan web applications it uses HTTP/S protocol but sometimes services like `whois` are using custom SQL databases as well, which is an ideal situation to produce unwanted problems because you have to take care of the sanitization of user input. The `whois` service, which I used for testing is working on a non-retired [HackTheBox.eu](https://hackthebox.eu) box, so I can’t do it very detailed, but you should be able to take advantage of the idea itself to create your custom proxies or just give a try with my tool `shiny-garbonzo` - an HTTP wrapper, which will invoke your client and pass to it the `sqlmap's` input imitating the legit HTTP communication. Let’s start by diving into how `sqlmap` works.
